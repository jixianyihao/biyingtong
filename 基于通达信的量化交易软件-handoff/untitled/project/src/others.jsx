...
function LiveTrading() {
  ...
  const [price, setPrice] = useState(1684.50);
  const [qty, setQty] = useState(100);
  const [code, setCode] = useState('600519');

  const bids = [
    { p: 1684.50, v: 342, cum: 342 }, { p: 1684.40, v: 128, cum: 470 },
    { p: 1684.30, v: 56, cum: 526 }, { p: 1684.20, v: 221, cum: 747 },
    { p: 1684.10, v: 89, cum: 836 },
  ];
  const asks = [
    { p: 1684.60, v: 187, cum: 187 }, { p: 1684.70, v: 98, cum: 285 },
    { p: 1684.80, v: 234, cum: 519 }, { p: 1684.90, v: 142, cum: 661 },
    { p: 1685.00, v: 412, cum: 1073 },
  ];

  const positions = [
    { code: '600519', name: '贵州茅台', qty: 200, cost: 1542.80, cur: 1684.50, from: 'AI-林园' },
    { code: '300750', name: '宁德时代', qty: 500, cost: 242.10, cur: 247.80, from: 'AI-浮游' },
    { code: '000858', name: '五粮液', qty: 800, cost: 152.40, cur: 158.42, from: '均线突破' },
    { code: '601899', name: '紫金矿业', qty: 3000, cost: 16.84, cur: 18.24, from: 'AI-林园' },
    { code: '600036', name: '招商银行', qty: 1000, cost: 41.20, cur: 42.18, from: '红利低波' },
    { code: '002594', name: '比亚迪', qty: 400, cost: 281.20, cur: 278.90, from: '网格套利' },
  ];

  const orders = [
    { t: '14:32:18', code: '300750', side: 'buy', qty: 200, price: 247.80, status: 'filled', from: 'AI-浮游' },
    { t: '14:28:04', code: '600030', side: 'sell', qty: 800, price: 22.14, status: 'filled', from: '均线突破' },
    { t: '14:24:12', code: '000001', side: 'buy', qty: 1000, price: 12.34, status: 'partial', filled: 620, from: '手动' },
    { t: '14:21:55', code: '601899', side: 'buy', qty: 1000, price: 17.89, status: 'filled', from: 'AI-林园' },
    { t: '14:08:22', code: '002594', side: 'sell', qty: 200, price: 279.50, status: 'cancelled', from: '手动' },
  ];
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '320px minmax(0,1fr) 340px',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minHeight: 0 }}>
        <div className="panel" style={{ padding: 14 }}>
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 3 }}>贵州茅台 · <span className="up mono">¥1,684.50</span> <span className="up">+2.34%</span></div>
          </div>
          <div style={{ marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              ...
              <input type="number" value={price} onChange={e => setPrice(+e.target.value)} className="mono" style={{
            ...
            <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
              {[['市价', 1684.50], ['对手价', 1684.60], ['涨停', 1852.90]].map(([l, v]) => (
                <span key={l} onClick={() => setPrice(v)} style={{
          ...
          <div style={{ padding: 10, background: 'var(--bg-2)', borderRadius: 4, marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 3 }}>
              ...
              <span className="mono" style={{ color: 'var(--text-hi)' }}>¥{fmt(price * qty)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 3 }}>
              <span style={{ color: 'var(--text-faint)' }}>手续费 (0.025%)</span>
              <span className="mono" style={{ color: 'var(--text-dim)' }}>¥{fmt(price * qty * 0.00025)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11 }}>
              <span style={{ color: 'var(--text-faint)' }}>可用 / 需要</span>
              <span className="mono" style={{ color: 'var(--text-hi)' }}>¥684,201 / ¥{fmt(price * qty)}</span>
            </div>
          </div>
        ...
        <div className="panel" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: '4px 0', flex: 1, overflow: 'auto' }}>
            {asks.slice().reverse().map((o, i) => (
              <DepthRow key={'a'+i} o={o} side="down" level={asks.length - i} maxCum={asks.at(-1).cum}/>
            ))}
            <div style={{ padding: '8px 12px', display: 'flex', justifyContent: 'space-between',
              ...
              <span className="mono up" style={{ fontSize: 16, fontWeight: 600 }}>1684.50</span>
              <span className="up" style={{ fontSize: 11 }}>+38.40 +2.34%</span>
            </div>
            {bids.map((o, i) => (
              <DepthRow key={'b'+i} o={o} side="up" level={i + 1} maxCum={bids.at(-1).cum}/>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="panel" style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        <div className="panel-head">
          ...
          <span className="mono" style={{ fontSize: 12, color: 'var(--text-hi)', fontWeight: 600 }}>¥2,163,012</span>
          <span className="mono up" style={{ fontSize: 11 }}>+8.4%</span>
        </div>
      ...
      <div onClick={e => e.stopPropagation()} className="panel" style={{
          ...
          <>
            <div style={{ padding: 20 }}>
              <div style={{ textAlign: 'center' }}>
                <div className="serif" style={{ fontSize: 18, color: 'var(--text-hi)' }}>贵州茅台</div>
                <div className="mono" style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 2 }}>{code}.SH</div>
              </div>
        ...
        {stage === 'done' && (
          <div style={{ padding: 44, textAlign: 'center' }}>
            ...
            <div className="mono" style={{ color: 'var(--text-faint)', marginTop: 4, fontSize: 11 }}>合同编号 BYT20260420143502</div>
