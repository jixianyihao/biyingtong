...
function TopBar({ active, onTweakOpen }) {
  ...
  return (
    <header style={{
      ...
      <div onClick={() => setTdxOpen(true)} title="点击查看通达信接口详情" style={{
      ...
      onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--up)'; e.currentTarget.style.background = 'var(--up-bg)'; }}
      onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--up-border)'; e.currentTarget.style.background = 'var(--bg-2)'; }}>
        <span className="live-dot" style={{ color: 'var(--up)' }}/>
        <span className="mono" style={{ fontSize: 10.5, color: 'var(--text-hi)', letterSpacing: '0.04em' }}>TDX · pytdx</span>
        <span style={{ width: 1, height: 10, background: 'var(--panel-border)' }}/>
        <span className="mono" style={{ fontSize: 10, color: 'var(--up)' }}>12ms</span>
        <span className="mono" style={{ fontSize: 9.5, color: 'var(--text-ghost)' }}>·</span>
        <span className="mono" style={{ fontSize: 10, color: 'var(--text-faint)' }}>行情主站#03</span>
        <span style={{ fontSize: 8, color: 'var(--text-ghost)', marginLeft: 2 }}>▾</span>
      </div>
  ...
  return (
    <footer style={{
      ...
      <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
        <span className="live-dot" style={{ color: 'var(--up)' }}/> pytdx v1.72 · Level-2
      </span>
      <span>tcp://119.147.212.81:7709</span>
      <span>sid #A3F07</span>
      <span>tick 1,842/s</span>
      <span>rtt 12ms</span>
      <span style={{ flex: 1 }}/>
      <span>Py 3.11 · pandas 2.2</span>
      <span>CPU 34%</span>
      <span>策略运行 7</span>
      <span>API 12,847/50,000</span>
      <span>2026-04-20 14:35:43 · 周一</span>
    </footer>
  );
}

Object.assign(window, { Sidebar, TopBar, StatusBar, RedLineBar, RedLineConfigModal, TDXPanel });
