...
```python
"""Load HS300 daily K-line from TDX into vnpy_sqlite.
...
def load_single_stock(symbol: str, start: datetime, end: datetime) -> int:
    """Fetch daily bars for `symbol` from TDX and save to vnpy_sqlite.
...
if __name__ == '__main__':
    # Running as script: load a tiny set for smoke-test
    now = datetime.now()
...
git commit -m "feat(p0): load_single_stock via vnpy-tdx into vnpy_sqlite"
```
